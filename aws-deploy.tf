...
resource "null_resource" "preparation" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo apt-get -qqy install time apt-transport-https ca-certificates curl gnupg2 software-properties-common build-essential libssl-dev",
...
#      "ssh-keygen -q -t rsa -f ~/.ssh/id_rsa -N '' -C for_git_checkout",
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "cd /srv/gisapp && ./gis.sh gs-shell --cmdfile init-geoserver.gs-shell",
    ]
  }
}
